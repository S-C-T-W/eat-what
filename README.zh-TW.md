...
### 🗺️ 搜邊度
圍住你嘅圓圈只係預設。可以**偏移**（向東 300 米，由你位置或者任何搜尋到嘅基點出發）、收窄做某個方向嘅 **90° / 180° 扇形**、最多 **5 個點**取聯集，或者沿 A→B 線搜一條**走廊**（獨立闊度滑桿，兩點最遠 8 公里）。覆蓋規劃器將任何形狀壓成 ≤ 3 次 Google 查詢，結果再喺前端按準確幾何重新篩一次；**地圖預覽**（MapLibre + OpenFreeMap 向量地圖，免金鑰）即時投影你將會搜嘅範圍——開 filter 嗰一刻攞一次定位，畫個藍點話你知自己喺邊。進階模式收埋喺 ⚙️ 後面，日常 filter 保持短小。
...
可安裝 PWA（離線外殼、自動更新 service worker）、iOS 26 式液態玻璃 tab bar、冇輸入縮放冇文字選取嘅原生手感、掃動手勢、完整深色模式、**四種介面語言**——English、廣東話（zh-HK）、台灣繁體中文（zh-TW）、日本語（ja）。中文按*地區*而唔係按字體配對：香港用戶見廣東話，台灣用戶見書面語，各有自己嘅 app 名（食乜好 · 呷什麼 · なに食べる？）；字串鍵同佔位符由測試強制對齊。
...
- **2026 年 8–9 月回合**——搜尋幾何（偏移／扇形／多點／走廊，≤ 3 次查詢嘅覆蓋規劃器）、地圖即時預覽、逐次到訪嘅食記同平均判決、連鎖店／快餐篩選、四種介面語言
...
- 地圖預覽會由 [OpenFreeMap](https://openfreemap.org)（`tiles.openfreemap.org`）攞向量地圖——係 app 入面唯一嘅第三方連線。佢只會知道你望緊邊一帶（tile 座標），永遠唔會知你嘅金鑰、紀錄或者準確定位；冇 cookie、冇帳號。想完全自主可以將 PMTiles 自行放上 R2，文件有寫。
